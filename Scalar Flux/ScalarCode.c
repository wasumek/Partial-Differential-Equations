...
void Plot3D(FILE * pFILE3D, double * l, double t);
...
void Flux_Calc(double * fluxP, double * fluxM, double * alpha, double * u, double * f) {
  for (int i=0; i < col; i++) {
    ...
    fluxP[a(0,i)] = (0.5)*(f[a(0,i+1)]+f[a(0,i)]) - (0.5)*alpha[i]*(u[a(0,i+1)]-u[a(0,i)]);
    fluxP[a(1,i)] = (0.5)*(f[a(1,i+1)]+f[a(1,i)]) - (0.5)*alpha[i]*(u[a(1,i+1)]-u[a(1,i)]);
    fluxP[a(2,i)] = (0.5)*(f[a(2,i+1)]+f[a(2,i)]) - (0.5)*alpha[i]*(u[a(2,i+1)]-u[a(2,i)]);
    
    fluxM[a(0,i)] = (0.5)*(f[a(0,i)]+f[a(0,i-1)]) - (0.5)*alpha[i-1]*(u[a(0,i)]-u[a(0,i-1)]);
    fluxM[a(1,i)] = (0.5)*(f[a(1,i)]+f[a(1,i-1)]) - (0.5)*alpha[i-1]*(u[a(1,i)]-u[a(1,i-1)]);
    fluxM[a(2,i)] = (0.5)*(f[a(2,i)]+f[a(2,i-1)]) - (0.5)*alpha[i-1]*(u[a(2,i)]-u[a(2,i-1)]);
...
void Plot3D(FILE * pFILE3D, double * l, double t){
  double h = 1/(float)N; // x is [0,1]
  for(int c=0; c<col; c++){
    fprintf(pFILE3D,"%.03lf\t %.03lf\t %.03lf\t \n",h*c,t,l[c]);
    //if((c+1)%row==0)printf("\n");
  }
  fprintf(pFILE3D,"\n");
}
